...
{
    NSMutableArray *ProgressList,*FilteredTasks;
    Boolean isFiltered;
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
{
    ProgressList=[Save_Retrieve_Class Retrieve_Progress_Data];
    [_tabl reloadData];
}
...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    {
        task=[ProgressList objectAtIndex:indexPath.row];
    }
...
-(void) tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        [view setMyTask:[ProgressList objectAtIndex:indexPath.row]];
    }
    [self presentViewController:view animated:YES completion:nil];
}
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        {
            t=[ProgressList objectAtIndex:indexPath.row];
            [ProgressList removeObjectAtIndex:indexPath.row];
        }
